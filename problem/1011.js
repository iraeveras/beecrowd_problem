...
export function problem(lines) {
    ...
    const pi = 3.14159
    const volume = (4.0/3) * pi * Math.pow(raio, 3) // que correponde a -> const volume = (4.0/3) * pi * raio ** 3
    console.log(`VOLUME = ${volume.toFixed(3)}`);
    document.querySelector(".input-resp").innerHTML += `<span>VOLUME = ${volume.toFixed(3)}</span><hr>`;
}